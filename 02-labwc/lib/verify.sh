...
verify_kitty_semantics() {
  ...
  grep -F 'initial_window_width 104c' "$kitty_path" >/dev/null || die "kitty config missing compact initial width"
  grep -F 'initial_window_height 28c' "$kitty_path" >/dev/null || die "kitty config missing compact initial height"
...
verify_shell_config_semantics() {
  ...
  grep -F 'show_always = false' "$LABWC_TARGET_HOME/.config/starship.toml" >/dev/null || die "starship.toml username policy is not compact"
  grep -F 'ssh_only = true' "$LABWC_TARGET_HOME/.config/starship.toml" >/dev/null || die "starship.toml hostname policy is not compact"
