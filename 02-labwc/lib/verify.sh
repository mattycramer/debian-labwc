...
verify_polkit_semantics() {
  ...
  grep -F 'wait_for_system_bus()' "$autostart_path" >/dev/null || die "labwc autostart missing system bus readiness helper"
  grep -F 'warm_polkit_service()' "$autostart_path" >/dev/null || die "labwc autostart missing polkit warmup helper"
  grep -F 'systemctl start polkit.service' "$autostart_path" >/dev/null || die "labwc autostart missing polkit service warmup"
...
verify_waybar_config_semantics() {
  ...
  grep -F '"format": "dGPU"' "$waybar_path" >/dev/null || die "waybar config missing Nvidia GPU launcher label"
...
verify_foot_semantics() {
  ...
  grep -F 'font=Noto Sans Mono:size=10.0' "$foot_path" >/dev/null || die "foot config missing the tuned base font size"
  ! grep -F 'font-bold=' "$foot_path" >/dev/null || die "foot config must not force a custom bold font"
  ! grep -F 'font-italic=' "$foot_path" >/dev/null || die "foot config must not force a custom italic font"
  ! grep -F 'font-bold-italic=' "$foot_path" >/dev/null || die "foot config must not force a custom bold italic font"
...
verify_kitty_semantics() {
  ...
  grep -F 'remember_window_size no' "$kitty_path" >/dev/null || die "kitty config must not override the configured default size"
  grep -F 'initial_window_width 960' "$kitty_path" >/dev/null || die "kitty config missing compact initial width"
  grep -F 'initial_window_height 620' "$kitty_path" >/dev/null || die "kitty config missing compact initial height"
  grep -F 'active_border_color #6dc4ed' "$kitty_path" >/dev/null || die "kitty config missing active border color"
  grep -F 'url_color #7dd3fc' "$kitty_path" >/dev/null || die "kitty config missing URL color"
...
verify_shell_config_semantics() {
  ...
  grep -F 'export LS_COLORS=' "$LABWC_TARGET_HOME/.bashrc" >/dev/null || die ".bashrc missing LS_COLORS palette"
  grep -F "alias ls='ls --color=auto --group-directories-first'" "$LABWC_TARGET_HOME/.bashrc" >/dev/null || die ".bashrc missing colorized ls alias"
  grep -F "alias ll='ls --color=auto --group-directories-first -alFh'" "$LABWC_TARGET_HOME/.bashrc" >/dev/null || die ".bashrc missing ll alias"
  grep -F "alias la='ls --color=auto --group-directories-first -A'" "$LABWC_TARGET_HOME/.bashrc" >/dev/null || die ".bashrc missing la alias"
  ...
  grep -F 'export LS_COLORS=' "$LABWC_TARGET_HOME/.zshrc" >/dev/null || die ".zshrc missing LS_COLORS palette"
  grep -F "alias ls='ls --color=auto --group-directories-first'" "$LABWC_TARGET_HOME/.zshrc" >/dev/null || die ".zshrc missing colorized ls alias"
  grep -F "alias ll='ls --color=auto --group-directories-first -alFh'" "$LABWC_TARGET_HOME/.zshrc" >/dev/null || die ".zshrc missing ll alias"
  grep -F "alias la='ls --color=auto --group-directories-first -A'" "$LABWC_TARGET_HOME/.zshrc" >/dev/null || die ".zshrc missing la alias"
  ...
  grep -F 'command_timeout = 1000' "$LABWC_TARGET_HOME/.config/starship.toml" >/dev/null || die "starship.toml missing command timeout"
  grep -F 'scan_timeout = 20' "$LABWC_TARGET_HOME/.config/starship.toml" >/dev/null || die "starship.toml missing scan timeout"
  grep -F '$username$hostname$directory$git_branch$git_status$git_state$cmd_duration' "$LABWC_TARGET_HOME/.config/starship.toml" >/dev/null || die "starship.toml prompt format is missing username, hostname, or directory modules"
  ...
  grep -F '[directory.substitutions]' "$LABWC_TARGET_HOME/.config/starship.toml" >/dev/null || die "starship.toml missing directory substitutions"
