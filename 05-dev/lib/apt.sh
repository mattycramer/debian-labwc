...
install_bootstrap_packages() {
  ...
  run_cmd env DEBIAN_FRONTEND=noninteractive APT_LISTCHANGES_FRONTEND=none apt install --no-install-recommends "${target_args[@]}" "${apt_args[@]}" "${BOOTSTRAP_PACKAGES[@]}"
}
...
install_dev_packages() {
  ...
  mapfile -t apt_args < <(apt_yes_args)
  mapfile -t target_args < <(apt_target_args)
  run_cmd env DEBIAN_FRONTEND=noninteractive APT_LISTCHANGES_FRONTEND=none apt install --no-install-recommends "${target_args[@]}" "${apt_args[@]}" "${DEV_PACKAGES[@]}"
}
