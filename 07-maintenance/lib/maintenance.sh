...
grub_btrfs_snapshot_kernel_parameters() {
  printf '%s\n' "${MAINTENANCE_ROOT_BTRFS_KERNEL_FLAGS} ${GRUB_NVME_FLAGS} ${GRUB_CGROUP_FLAGS} ${GRUB_BLACKLIST_FLAGS} ${GRUB_HARDENING_FLAGS} ${GRUB_ASPM_FLAGS} bootprofile=${GRUB_BOOTPROFILE_DEFAULT} ${GRUB_PROFILE_DEFAULT_FLAGS}"
}
