...
install_dbus_runtime_packages() {
  ...
  retry_cmd 3 env DEBIAN_FRONTEND=noninteractive APT_LISTCHANGES_FRONTEND=none \
    apt install --no-install-recommends -o DPkg::Lock::Timeout=60 "${apt_args[@]}" "${DBUS_RUNTIME_PACKAGES[@]}"
  if [[ "${DBUS_BROKER_INSTALL_METHOD:-source}" == "source" ]]; then
    retry_cmd 3 env DEBIAN_FRONTEND=noninteractive APT_LISTCHANGES_FRONTEND=none \
      apt install --no-install-recommends -o DPkg::Lock::Timeout=60 "${apt_args[@]}" "${DBUS_SOURCE_BUILD_PACKAGES[@]}"
  fi
}
