...
verify_greeter_contract() {
  ...
  grep -F 'log_file="/var/log/regreet/log"' "/usr/local/bin/labwc-greeter-regreet" >/dev/null || {
    die "greeter launcher lost the managed regreet log path"
  }
  grep -F 'exec >>"$log_file" 2>&1' "/usr/local/bin/labwc-greeter-regreet" >/dev/null || {
    die "greeter launcher no longer captures regreet output to the managed log file"
  }
