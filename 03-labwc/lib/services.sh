...
managed_broker_system_unit_path() {
  printf '%s\n' "/etc/systemd/system/dbus.service"
}

managed_broker_user_unit_path() {
  printf '%s\n' "/etc/systemd/user/dbus.service"
}

require_managed_dbus_broker_units() {
  local system_unit user_unit
  system_unit="$(managed_broker_system_unit_path)"
  user_unit="$(managed_broker_user_unit_path)"

  require_file "/usr/bin/dbus-broker"
  require_file "/usr/bin/dbus-broker-launch"
  require_file "$system_unit"
  require_file "$user_unit"
  grep -F 'ExecStart=/usr/bin/dbus-broker-launch --scope system' "$system_unit" >/dev/null || {
    die "managed system dbus.service is missing the dbus-broker-launch --scope system contract; stage 01-dbus-broker first"
  }
  grep -F 'ExecStart=/usr/bin/dbus-broker-launch --scope user' "$user_unit" >/dev/null || {
    die "managed user dbus.service is missing the dbus-broker-launch --scope user contract; stage 01-dbus-broker first"
  }
}
